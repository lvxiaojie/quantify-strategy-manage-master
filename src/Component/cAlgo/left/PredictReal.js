...
let TodoList = React.createClass({
    ...
    // let jquery_jd = todolist[i].id + '_accuracy';
    // $('#' + jquery_jd).text(todolist[i].accuracy + '%');
...
let ListTodo = React.createClass({
    getInitialState: function() {
        return {
            delName: '',
            delId: '',
    ...
    render: function() {
        ...
        let o = this;
        // let flag = Prop.username != undefined ? Prop.username : Prop.exchange;
        let modalId = this.props.todo.length > 0 ? this.props.todo[0].id : '';
        return (
            <div style={{marginTop:'0px'}}>
            {this.props.todo.length>0?<i className='predict smallfont' style={{marginLeft:'15px',}}>实盘预测</i>:null}
            <ul style={{overflow:'hidden',padding:0}}>
            {
                this.props.todo.map(function (x,index) {
                let classN = 'liStyle2 smallfont ' + flag;
                let hisProfit_id = 'HisProfit_' + x.id;
                let menu_id = 'menu_' + x.id;
                let astyle = {
                    color: x.status == -1 ? "#ff0000" : "#CCCCFF",
                    cursor: "default",
                    marginLeft: '10px',
                    position: 'relative',
                };
                ...
                let dataArray = $.map(x.model_info, function(value, name) {
                    return [name];
                });
                    return (
                        <li className={classN}  key={index} id={hisProfit_id}>
                            ...
                            data-toggle="collapse" data-target={"#" + menu_id}></i>
                              <i  title={x.status==-1?x.error:""} style={astyle}
                              ><i style={{fontSize:'14px'}}>{x.name}</i>                         
                             </i>
                            ...
                            > </span>
                              ...
                              <i style={{color:'#f15923'}} id={x.id+'_accuracy'}>{x.accuracy}%</i>
                              :null}
                              </span>
                              <i className="smallfont" style={{marginLeft:"5px",color:x.status==-1?"#ff0000":"#fff",cursor:'pointer'}} title={x.error} onClick={(e)=>o.showError(x.error)}>{x.flag}</i>
                              <i  className="smallfont" style={{float:'right',marginRight:'3%'}}><span style={{color:'#fff'}}>{x.endTime}</span></i>
                                ...
                                <table style={{marginBottom:'0px',width:'96%',marginLeft:'2%',backgroundColor:'#3b3b3b'}} 
                                  ...
                                  <tbody className="model_info_tbody">
                                                ...
                                                <tr>
                                                     ...
                                                     <td
                                                     style={{textDecoration: 'underline',border: '1px solid #666',width: '65%',color:'#f1825f',cursor:'pointer'}} 
                                 ...
                                 <table style={{marginBottom:'0px',width:'96%',marginLeft:'2%',backgroundColor:'#3b3b3b'}} 
                                  ...
                                  <tbody className="model_info_tbody">
                                    ...
                                    {
                                        dataArray.map(function(y,index2){
                                            return (
                                                <tr key={index2}>
...
{/*                               <span className="iblock smallfont">模型名称：{x.file_info.string_data.model_name}</span>
                                  <span className="iblock smallfont">模型版本：{x.file_info.string_data.model_version}</span>
                                  <span className="iblock smallfont">模型作者：{x.file_info.string_data.model_author}</span>
                                  <span className="iblock smallfont">模型日期：{x.file_info.string_data.model_date}</span>
                                  <span className="iblock smallfont">模型描述：{x.file_info.string_data.model_describe}</span>
                                  <span className="iblock smallfont">模型目标：{x.file_info.string_data.model_target}</span>*/}
...
class PredictReal extends Component {
    constructor(props) {
        super(props);
        this.state = {
            StrategyList: [],
...
export default connect(mapStateToProps)(PredictReal);